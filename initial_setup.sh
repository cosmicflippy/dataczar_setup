...
echo "Command 3: cd ~/Downloads/ydotool/build && sudo mv ydotoold.serice /etc/systemd/system/"
echo "Command 4: sudo systemctl enable ydotoold && sudo systemctl start ydotoold"
echo "Command 5: sudo echo 'export YDOTOOL_SOCKET=/tmp/.ydotool_socket' >> ~/.profile"

echo "Once done with all of that, reboot and run the script."